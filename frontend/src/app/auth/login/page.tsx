    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-16 bg-background">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="mt-10 space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required placeholder="you@example.com or 9876543210"
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-white/10 bg-white/5 text-accent focus:ring-accent/20" />
          ...
          <div className="mt-8 flex items-center gap-4"><div className="flex-1 h-px bg-white/5" /><span className="text-xs text-white/20">or</span><div className="flex-1 h-px bg-white/5" /></div>
          ...
          <p className="mt-8 text-center text-sm text-white/30">Don&apos;t have an account? <Link href="/auth/register" className="text-accent hover:underline font-medium">Sign Up</Link></p>
        </motion.div>
      </div>
    </div>
  );
